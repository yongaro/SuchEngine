...
glPipeline::glPipeline(const string& fileName) {
...
void glPipeline::init(glm::mat4* m, glm::mat4* v, glm::mat4* p,glm::vec3* vp,glm::vec3* lp, glm::mat4* mvp){
...
std::string glPipeline::LoadShader(const std::string& fileName){
...
GLuint glPipeline::CreateShader(const std::string& text, unsigned int type){
...
GLuint glPipeline::getProg()const{ return programID; }
void glPipeline::setColorFlag(GLint f){ colorFlag = f; }
